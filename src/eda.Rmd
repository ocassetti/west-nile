...
for( i in 20:34){
  ...
  weatherLagged[, i] <- if_else(is.na(xVal), mean(xVal, na.rm = TRUE), xVal)
}
...
speciesSummary <- wnvAgg %>%  group_by(Week, Year, Species, WnvPresent) %>% summarise(NumMosquitos=sum(NumMosquitos))

ggplot(speciesSummary) + geom_bar(aes(x=Week, y=NumMosquitos, fill=WnvPresent), stat = "identity") + facet_wrap(~Species+Year)
...
augmentTestData <- function(df, weatherStation = matrix(c( -87.933 , 41.995, -87.752, 41.786), ncol = 2, byrow = TRUE)){
  ...
  group_by(Date, Species, SatelliteTrap, Block, Latitude, Longitude, AddressAccuracy) %>% summarise(Tcount=length(SatelliteTrap))
